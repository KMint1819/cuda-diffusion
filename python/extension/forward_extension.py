...
norm_weight = load_data(data_dir / 'norm-weight.txt', (n_channels,)).cuda()
norm_bias = load_data(data_dir / 'norm-bias.txt', (n_channels,)).cuda()
qkv_weight = load_data(data_dir / 'qkv-weight.txt', (n_channels * 3, n_channels, 1)).cuda()
qkv_bias = load_data(data_dir / 'qkv-bias.txt', (n_channels * 3,)).cuda()
proj_out_weight = load_data(data_dir / 'proj_out-weight.txt', (n_channels, n_channels, 1)).cuda()
proj_out_bias = load_data(data_dir / 'proj_out-bias.txt', (n_channels,)).cuda()
